...
class Minging():
    ...
    def Etest_Unit(self):
        ...
        for Etest in self.cfg.Etest:
            if self.cfg.Type in ['typ','sens']:
                temp += [self.unit(Etest)]
            else:
                temp += [self.unit(Etest)+'_median', self.unit(Etest)+'_sigma']
        return temp
    ...
    def median_std_mis(self,listA,listB):
        import json
        ...
        if type == 'Delta':
            list = [A-B for A,B in zip(listA,listB)]
        elif type == 'Ratio':
            list = [(A-B)/((A+B)/2) for A,B in zip(listA,listB)]
        ...
        for fname in os.listdir(self.cfg.Folder):
            ...
            if m is not None:
                if m.group(1) in postfix_list:
                    os.remove(self.cfg.Folder + '/' + fname)
